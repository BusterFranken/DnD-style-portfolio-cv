...
// Provider-agnostic via env: any OpenAI-compatible endpoint works (OpenAI,
// Mistral La Plateforme, etc.) as long as it supports json_schema structured
// output. Defaults preserve the original OpenAI setup.
...
PROFICIENCIES (never leave empty):
- armor: List 2-4 professional "armor" types (e.g., "Legal Compliance", "Data Privacy", "Risk Management", "Contract Negotiation")
- weapons: List 3-5 professional "weapons"/tools (e.g., "Python", "Excel", "Figma", "SQL", "PowerPoint", "Slack")
- tools: List 2-4 professional tool proficiencies (e.g., "Git", "Jira", "Notion", "Google Analytics", "Photoshop")

CAMPAIGNS vs SIDE QUESTS:
- CAMPAIGNS: All full-time jobs AND academic degrees (university, master's, PhD). Each multi-year experience = its own campaign.
- SIDE QUESTS: ONLY volunteer work, board positions, part-time jobs, freelance gigs, or brief engagements (<1 year or part-time)

SPELLS: Generate 2-3 cantrips and 2-4 spells per level (1-3). Map professional abilities to D&D spells.

DEFENSES & CONDITIONS: Generate 2-4 of each (never empty).

ACTIONS: All fields must be filled (attackBonus=modifier+proficiency, damage=dice like "2d6+3").
...
async function generateFullSheet(cvText, onProgress) {
  const client = new OpenAI({
    // Strip ALL whitespace (not just trim): a key pasted with internal line
    // breaks (from editor wrapping) puts a newline mid-header, which fails the
    // request at the transport layer ("Connection error."). API keys never
    // contain whitespace, so this is safe and fully corrects a mangled paste.
